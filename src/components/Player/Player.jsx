...
export default function Player({ option, style, getInstance, movieData, episodeNav }) {
    const artRef = useRef();
    const { user } = useAuth();
    const navigate = useNavigate();
    const saveTimeout = useRef(null);
    const dataRef = useRef({ user, movieData });
    const episodeNavRef = useRef(episodeNav);
    ...
    useEffect(() => {
        episodeNavRef.current = episodeNav;
    }, [episodeNav]);
    ...
    useEffect(() => {
        ...
        // --- Build controls array (including episode nav if applicable) ---
        const buildControls = () => {
            const baseControls = [
                {
                    name: 'fast-backward',
            ...
            // --- NÚT CHUYỂN TẬP ---
            const epNav = episodeNavRef.current;
            if (epNav && epNav.episodes && epNav.episodes.length > 1) {
                const epList = epNav.episodes;
                const curIdx = epList.findIndex((ep) => ep.slug === epNav.currentEpisodeSlug);
                const hasPrev = curIdx > 0;
                const hasNext = curIdx < epList.length - 1;
                ...
                baseControls.push({
                    ...
                    position: 'right',
                    index: 1,
                    html: `<div class="ep-nav-control ${!hasPrev ? 'ep-nav-disabled' : ''}" title="Tập trước">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width="14" height="14" fill="currentColor"><path d="M267.5 440.6c9.5 7.9 22.8 9.7 34.1 4.4s18.4-16.6 18.4-29V96c0-12.4-7.2-23.7-18.4-29s-24.5-3.6-34.1 4.4l-192 160c-7.3 6.1-11.5 15.1-11.5 24.6s4.2 18.5 11.5 24.6l192 160z"/></svg>
                    </div>`,
                    tooltip: 'Tập trước',
                    click: function () {
                        const nav = episodeNavRef.current;
                        if (!nav) return;
                        const list = nav.episodes;
                        const idx = list.findIndex((ep) => ep.slug === nav.currentEpisodeSlug);
                        if (idx > 0) {
                            navigate(`/xem-phim/${nav.movieSlug}/${list[idx - 1].slug}`);
                        }
                    },
                });
                ...
                baseControls.push({
                    ...
                    position: 'right',
                    index: 2,
                    html: `<div class="ep-nav-control ${!hasNext ? 'ep-nav-disabled' : ''}" title="Tập sau">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width="14" height="14" fill="currentColor"><path d="M52.5 440.6c-9.5 7.9-22.8 9.7-34.1 4.4S0 428.4 0 416V96C0 83.6 7.2 72.3 18.4 67s24.5-3.6 34.1 4.4l192 160c7.3 6.1 11.5 15.1 11.5 24.6s-4.2 18.5-11.5 24.6l-192 160z"/></svg>
                    </div>`,
                    tooltip: 'Tập sau',
                    click: function () {
                        const nav = episodeNavRef.current;
